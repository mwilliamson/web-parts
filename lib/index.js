...
function renderer(options) {
    ...
    function render(name, context) {
        ...
        if (partDefinition) {
            ...
            return q.reject(new Error("Unknown part: " + name));
        }
    }
        ...
        return q.all(renderedPartsPromise).then(function(renderedParts) {
            if (partDefinition.template) {
                ...
                renderedParts.forEach(function(renderedPart) {
                    var filledHole = renderedPartToFilledHole(renderedPart);
                    holeContents[filledHole.name] = filledHole.value;
                });
                
                var holeRegex = /<!--\s*HOLE:\s*(\S+)\s*-->/g;
                return partDefinition.template.fillHoles(holeContents);
            } else {
                var wrappedParts = renderedParts.map(renderedPartToFilledHole);
